'use client';

import { useFormState, useFormStatus } from 'react-dom';
import { authenticate } from '@/app/lib/actions';

function LoginButton() {
    const { pending } = useFormStatus();

    return (
        <button aria-disabled={pending} className="bg-[#010E21] text-[#E7C7A1] rounded-xl pt-3 py-3 mt-3 hover:bg-[#445a7e] transition-all">
            LOGIN
        </button>
    );
}

export default function LoginForm() {
    const [errorMessage, dispatch] = useFormState(authenticate, undefined);

    return (
        <div className="bg-[#E7C7A1] flex rounded-2xl shadow-lg min-h-96 min-w-fit">
            <div className="px-16 pt-10 py-10">
                <h2 className="font-bold text-2xl ">Login</h2>
                <p className="">Se já é um cliente, realize o Login</p>

                {/* Formulario */}

                {/* Email */}
                <form action={dispatch} className="flex flex-col">

                    <label className="input input-bordered flex items-center gap-2 mt-3">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="w-4 h-4 opacity-70"><path d="M2.5 3A1.5 1.5 0 0 0 1 4.5v.793c.026.009.051.02.076.032L7.674 8.51c.206.1.446.1.652 0l6.598-3.185A.755.755 0 0 1 15 5.293V4.5A1.5 1.5 0 0 0 13.5 3h-11Z" /><path d="M15 6.954 8.978 9.86a2.25 2.25 0 0 1-1.956 0L1 6.954V11.5A1.5 1.5 0 0 0 2.5 13h11a1.5 1.5 0 0 0 1.5-1.5V6.954Z" /></svg>
                        <input
                            type="text"
                            className="grow"
                            placeholder="Email"
                            name="email"
                            id="email"
                            required
                        />
                    </label>

                    {/* Senha */}
                    <label className="input input-bordered flex items-center gap-2 mt-3">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="w-4 h-4 opacity-70"><path fillRule="evenodd" d="M14 6a4 4 0 0 1-4.899 3.899l-1.955 1.955a.5.5 0 0 1-.353.146H5v1.5a.5.5 0 0 1-.5.5h-2a.5.5 0 0 1-.5-.5v-2.293a.5.5 0 0 1 .146-.353l3.955-3.955A4 4 0 1 1 14 6Zm-4-2a.75.75 0 0 0 0 1.5.5.5 0 0 1 .5.5.75.75 0 0 0 1.5 0 2 2 0 0 0-2-2Z" clipRule="evenodd" /></svg>
                        <input
                            type="password"
                            className="grow"
                            placeholder="Senha"
                            name="password"
                            id="password"
                            required
                        />
                    </label>
                    <LoginButton />
                    <div
                        className="flex h-8 items-end space-x-1"
                        aria-live="polite"
                        aria-atomic="true"
                    >
                        {errorMessage && (
                            <>
                                <p className="text-sm text-red-500">{errorMessage}</p>
                            </>
                        )}
                    </div>
                </form>


                {/* Esqueceu a senha / não possui conta */}

                <div className="mt-7 grid grid-cols-3 items-center text-black">
                    <hr className="border-black" />
                    <p className="text-center">OU</p>
                    <hr className="border-black" />
                </div>

                <p className="mt-7 text-xs border-black border-b py-4">Esqueceu a senha? <span>clique aqui</span></p>

                <div className=" py-4 text-xs flex justify-between">
                    <p className="pt-3 py-3">Não possui conta ainda?</p>
                    <a href="cadastro"><button className=" rounded-xl pt-3 p-5 py-3 justify-center bg-[#010E21] text-[#E7C7A1] hover:bg-[#445a7e] transition-all">Registre-se</button></a>
                </div>
            </div>
        </div>
    );
}

